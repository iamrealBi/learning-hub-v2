...
Trước khi học từng lớp cụ thể, bạn phải nắm **hệ phân cấp interface**. Đây là thứ quyết định bạn viết API tốt hay xấu: **nhận tham số kiểu interface hẹp nhất mà bạn thật sự cần**, trả về kiểu vừa đủ. Mục này giới thiệu **từng interface một**, mỗi cái có định nghĩa riêng + ví dụ tối thiểu riêng; sơ đồ và bảng tổng kết đặt ở cuối (1.9).

### 1.1 `IEnumerable<T>` — duyệt tuần tự, một chiều

`IEnumerable<T>` là interface chỉ cho phép **duyệt qua từng phần tử một lần, theo một chiều** (thông qua `foreach`), không có `Count`, không có chỉ số.

```csharp title="IEnumerable<T>: tham số rộng nhất"
// test:run
...
Console.WriteLine(Tong(new[] { 1, 2, 3 }));           // 6 — mảng
Console.WriteLine(Tong(new List<int> { 4, 5 }));      // 9 — List
```

Đây là interface **gốc** — mọi collection generic khác trong .NET đều trực tiếp hoặc gián tiếp cài `IEnumerable<T>`, nên nó là mẫu số chung cho LINQ và `foreach`.

### 1.2 `ICollection<T>` — thêm/bớt phần tử, biết số lượng

`ICollection<T>` mở rộng `IEnumerable<T>` bằng khả năng **biết `Count`** và **thêm/xoá/kiểm tra tồn tại** phần tử, không quan tâm thứ tự hay chỉ số.

```csharp title="ICollection<T>: cần thêm/bớt, không cần chỉ số"
// test:run
static void ThemNeuChuaCo(ICollection<string> xs, string gtri)
{
    if (!xs.Contains(gtri)) xs.Add(gtri);
}

var ds = new List<string> { "a" };
ThemNeuChuaCo(ds, "a");    // đã có -> không thêm
ThemNeuChuaCo(ds, "b");    // chưa có -> thêm
Console.WriteLine(string.Join(",", ds));   // a,b
```

### 1.3 `IList<T>` — truy cập & chèn theo chỉ số

`IList<T>` mở rộng `ICollection<T>` bằng **chỉ số**: đọc/ghi qua `this[int]`, `Insert`, `RemoveAt`, `IndexOf`.

```csharp title="IList<T>: cần truy cập/chèn theo chỉ số"
// test:run
static void ChenDauNeuTrong(IList<int> xs, int gtri)
{
    if (xs.Count == 0) xs.Insert(0, gtri);
}

IList<int> ds = new List<int>();
ChenDauNeuTrong(ds, 99);
Console.WriteLine(ds[0]);   // 99
```

### 1.4 `IDictionary<K,V>` — tra cứu + sửa theo khoá

`IDictionary<K,V>` là bảng ánh xạ **khoá → giá trị** cho phép **đọc, ghi, thêm, xoá** qua khoá (`this[K]`, `Add(k,v)`, `Remove(k)`, `Keys`, `Values`).

```csharp title="IDictionary<K,V>: tham số chuẩn cho 'một bảng ánh xạ'"
// test:run
static void CapNhat(IDictionary<string, int> bang, string k, int v)
{
    bang[k] = v;   // hoạt động với bất kỳ cài đặt IDictionary nào, không khoá cứng Dictionary<K,V>
}

IDictionary<string, int> tuoi = new Dictionary<string, int>();
CapNhat(tuoi, "An", 20);
Console.WriteLine(tuoi["An"]);   // 20
```

### 1.5 `ISet<T>` — phép toán tập hợp

`ISet<T>` mở rộng `ICollection<T>` bằng các **phép toán tập hợp** (hợp, giao, hiệu) như `UnionWith`, `IntersectWith`, `ExceptWith`.

```csharp title="ISet<T>: cần phép toán tập hợp"
// test:run
static void HopVao(ISet<int> dich, ISet<int> nguon)
{
    dich.UnionWith(nguon);   // chỉ ISet<T> mới có UnionWith, ICollection<T> thì không
}

ISet<int> a = new HashSet<int> { 1, 2 };
HopVao(a, new HashSet<int> { 2, 3 });

var ketQua = new List<int>(a);
ketQua.Sort();   // HashSet không đảm bảo thứ tự -> sắp lại bằng List.Sort() để in ổn định
Console.WriteLine(string.Join(",", ketQua));   // 1,2,3
```

### 1.6 `IReadOnlyCollection<T>` — chỉ đọc, biết số lượng

`IReadOnlyCollection<T>` mở rộng `IEnumerable<T>` bằng `Count`, nhưng **không có** phương thức nào để sửa (`Add`/`Remove`).

```csharp title="IReadOnlyCollection<T>: biết số lượng, cam kết không sửa"
// test:run
static void InSoLuong(IReadOnlyCollection<int> xs)
{
    Console.WriteLine($"Có {xs.Count} phần tử");   // đọc Count nhưng không thể xs.Add(...)
}

InSoLuong(new List<int> { 1, 2, 3 });   // Có 3 phần tử
```

### 1.7 `IReadOnlyList<T>` — chỉ đọc theo chỉ số

`IReadOnlyList<T>` mở rộng `IReadOnlyCollection<T>` bằng `this[int]` **chỉ đọc** — truy cập theo chỉ số mà không thể gán lại phần tử qua interface này.

```csharp title="IReadOnlyList<T>: đọc theo chỉ số, không sửa"
// test:run
static int PhanTuDau(IReadOnlyList<int> xs) => xs[0];   // đọc được, nhưng xs[0] = 9 sẽ KHÔNG biên dịch

var ds = new List<int> { 10, 20, 30 };
Console.WriteLine(PhanTuDau(ds));   // 10
```

### 1.8 `IReadOnlyDictionary<K,V>` — tra cứu theo khoá, chỉ đọc

`IReadOnlyDictionary<K,V>` mở rộng `IReadOnlyCollection<KeyValuePair<K,V>>` bằng `Keys`, `Values`, `this[K]` (chỉ đọc), `TryGetValue`, `ContainsKey` — dùng khi trả cấu hình/bảng tra cứu ra ngoài mà không cho sửa.

```csharp title="IReadOnlyDictionary<K,V>: trả bảng tra cứu chỉ đọc"
// test:run
static bool CoKhoa(IReadOnlyDictionary<string, int> bang, string k) => bang.ContainsKey(k);

IReadOnlyDictionary<string, int> cauHinh = new Dictionary<string, int> { ["timeout"] = 30 };
Console.WriteLine(CoKhoa(cauHinh, "timeout"));   // True — đọc được, không có Add/Remove
```

### 1.9 Tổng kết: cây phân cấp và bảng so sánh

Sau khi đã thấy từng interface riêng lẻ, đây là **toàn cảnh** hệ phân cấp:
...
```csharp title="Tự viết IEqualityComparer<T>: so khớp theo một phần đối tượng"
// test:run
var sp1 = new SanPham { Ma = "SP001", Ten = "Áo thun" };
var sp2 = new SanPham { Ma = "sp001", Ten = "Áo thun (đã đổi tên)" };   // Ma khác hoa/thường, Ten khác

var theoMa = new Dictionary<SanPham, int>(new SoSanhTheoMa());
theoMa[sp1] = 100;
Console.WriteLine(theoMa.ContainsKey(sp2));   // True — chỉ so Ma, không phân biệt hoa/thường, bỏ qua Ten
...
// Tự viết đầy đủ IEqualityComparer<T>: chỉ so theo Ma, không phân biệt hoa/thường
class SoSanhTheoMa : IEqualityComparer<SanPham>
{
    public bool Equals(SanPham? a, SanPham? b)
    {
        if (ReferenceEquals(a, b)) return true;
        if (a is null || b is null) return false;
        return string.Equals(a.Ma, b.Ma, StringComparison.OrdinalIgnoreCase);
    }

    public int GetHashCode(SanPham obj) =>
        obj.Ma.ToUpperInvariant().GetHashCode();
}
```

**Điểm cốt lõi:** `SoSanhTheoMa` không đụng vào lớp `SanPham` gốc — nó là một "chính sách so sánh" tách biệt, truyền vào constructor của `Dictionary`/`HashSet`. Vẫn phải tôn trọng đúng hợp đồng Equals/GetHashCode (mục 4.2): hai đối tượng `Equals` trả `true` thì `GetHashCode` phải trả cùng giá trị.
...
// HashSet không đảm bảo thứ tự -> chép ra List rồi Sort() để in ổn định
static string SapVaIn(HashSet<int> tap)
{
    var ds = new List<int>(tap);
    ds.Sort();
    return string.Join(",", ds);
}

Console.WriteLine(SapVaIn(hop));       // 1,2,3,4,5,6
Console.WriteLine(SapVaIn(giao));      // 3,4
Console.WriteLine(SapVaIn(hieu));      // 1,2
Console.WriteLine(SapVaIn(doiXung));   // 1,2,5,6
...
!!! note "Còn một họ phương thức nữa, để dành cho chương LINQ"
    Ngoài `UnionWith`/`IntersectWith`/`ExceptWith` (đột biến tại chỗ trên `HashSet<T>`) ở trên, chương LINQ (sau chương này) sẽ giới thiệu thêm các phương thức **không đột biến**, trả về kết quả mới, cho phép so sánh theo một khoá trích ra thay vì cả phần tử. Chưa cần dùng đến chúng ở đây.
...
```csharp title="ConcurrentQueue<T>: FIFO an toàn đa luồng"
// test:run
using System.Collections.Concurrent;
...
if (hangDoi.TryDequeue(out var x)) Console.WriteLine(x);   // 1 — không ném nếu rỗng, trả bool
Console.WriteLine(hangDoi.Count);                          // 1
```

```csharp title="ConcurrentStack<T>: LIFO an toàn đa luồng"
// test:run
using System.Collections.Concurrent;

var ngan = new ConcurrentStack<int>();
ngan.Push(1);
ngan.Push(2);

if (ngan.TryPop(out var top)) Console.WriteLine(top);   // 2 — không ném nếu rỗng, trả bool
Console.WriteLine(ngan.Count);                          // 1
```

```csharp title="BlockingCollection<T>: bọc collection concurrent, thêm khả năng chặn"
// test:run
using System.Collections.Concurrent;

var hangCho = new BlockingCollection<int>(boundedCapacity: 2);   // tối đa 2 phần tử cùng lúc
hangCho.Add(1);
hangCho.Add(2);

// Đã đầy (2/2) -> TryAdd chờ tối đa 50ms rồi bỏ cuộc thay vì chặn vô hạn
Console.WriteLine(hangCho.TryAdd(3, TimeSpan.FromMilliseconds(50)));   // False — hết giờ chờ vì đầy

Console.WriteLine(hangCho.Take());   // 1 — lấy ra theo FIFO, có chỗ trống trở lại
Console.WriteLine(hangCho.Take());   // 2

hangCho.CompleteAdding();            // báo hiệu "không thêm nữa" cho luồng tiêu thụ
Console.WriteLine(hangCho.TryTake(out _));   // False — rỗng và đã CompleteAdding, không còn gì để lấy
```

!!! danger "Add()/Take() có thể chặn vô hạn nếu dùng sai"
    `hangCho.Add(x)` trên collection **đã đầy** (đạt `boundedCapacity`) sẽ **chặn (block) luồng gọi** cho đến khi có chỗ trống — nếu không luồng nào gọi `Take()` để giải phóng chỗ, luồng gọi `Add` treo **vĩnh viễn**. Tương tự, `Take()` trên collection **rỗng** mà chưa `CompleteAdding()` cũng chặn vô hạn chờ phần tử mới. Dùng `TryAdd(item, timeout)`/`TryTake(out item, timeout)` khi cần giới hạn thời gian chờ thay vì chặn mãi.

---

## 7. Các collection sắp xếp (Sorted)

Ba lớp "Sorted" giữ phần tử **theo thứ tự khoá** thay vì thứ tự chèn. Mục này giới thiệu **từng lớp riêng** với ví dụ tối thiểu, rồi mới tổng kết bằng bảng so sánh (7.5).

### 7.1 `SortedDictionary<K,V>` — cây đỏ-đen, chèn/xoá nhanh

`SortedDictionary<K,V>` là bảng ánh xạ khoá → giá trị, bên trong là **cây đỏ-đen (balanced BST)**, luôn **duyệt theo thứ tự khoá tăng dần** bất kể thứ tự chèn.

```csharp title="SortedDictionary<K,V>: duyệt luôn theo thứ tự khoá"
// test:run
var sd = new SortedDictionary<int, string>();
sd[3] = "ba"; sd[1] = "mot"; sd[2] = "hai";   // chèn theo thứ tự 3,1,2

foreach (var kv in sd) Console.Write($"{kv.Key}={kv.Value} ");
Console.WriteLine();                     // 1=mot 2=hai 3=ba — LUÔN theo khoá tăng dần
```

### 7.2 `SortedList<K,V>` — hai mảng song song, liền khối

`SortedList<K,V>` cũng là bảng ánh xạ khoá → giá trị sắp theo khoá, nhưng bên trong là **hai mảng song song** (khoá + giá trị), cho phép **truy cập theo index** ngoài truy cập theo khoá.

```csharp title="SortedList<K,V>: truy cập theo cả khoá lẫn index"
// test:run
var sl = new SortedList<int, string>();
sl[3] = "c"; sl[1] = "a"; sl[2] = "b";   // chèn theo thứ tự 3,1,2

Console.WriteLine(sl.Keys[0]);           // 1 — truy cập theo INDEX (mảng khoá đã sắp xếp)
Console.WriteLine(sl.Values[2]);         // c — truy cập theo INDEX (mảng giá trị tương ứng)
Console.WriteLine(sl[2]);                // b — truy cập theo KHOÁ như Dictionary
```

### 7.3 `SortedSet<T>` — tập hợp có sắp xếp

`SortedSet<T>` là phiên bản "chỉ khoá, không giá trị" của `SortedDictionary`: giữ các phần tử **duy nhất** và **luôn sắp xếp**, bên trong cũng là cây đỏ-đen.

```csharp title="SortedSet<T>: khử trùng + sắp xếp + truy vấn khoảng"
// test:run
var ss = new SortedSet<int> { 5, 1, 3, 3, 2 };   // sắp xếp + khử trùng tự động
...
### 7.4 Lỗi khi khoá không so sánh được: `InvalidOperationException`

Cả ba lớp Sorted đều cần **so sánh** hai khoá để giữ thứ tự. Nếu kiểu khoá **không** cài `IComparable<T>` (hoặc `IComparable`) và bạn **không** truyền `IComparer<T>` thay thế, .NET ném lỗi ngay khi thêm phần tử thứ hai (lúc thật sự cần so sánh hai khoá với nhau). Với `SortedList<K,V>`, lỗi cụ thể là **`InvalidOperationException`** ("Failed to compare two elements in the array"), vì bên trong nó gọi `Array.Sort`/`Array.BinarySearch` trên mảng khoá.

```csharp title="Khoá không IComparable + không IComparer -> InvalidOperationException"
// test:run
try
{
    var sl = new SortedList<DiemKhongSanhDuoc, string>();
    sl.Add(new DiemKhongSanhDuoc(1, 1), "a");
    sl.Add(new DiemKhongSanhDuoc(2, 2), "b");   // cần so sánh với phần tử đã có -> nổ
}
catch (InvalidOperationException ex)
{
    Console.WriteLine($"Nổ: {ex.Message}");   // Failed to compare two elements in the array.
}

class DiemKhongSanhDuoc
{
    public int X, Y;
    public DiemKhongSanhDuoc(int x, int y) { X = x; Y = y; }
    // KHÔNG cài IComparable<DiemKhongSanhDuoc> -> Sorted collection không biết so sánh thế nào
}
```

!!! note "SortedDictionary/SortedSet: cùng nguyên nhân, khác kiểu exception"
    `SortedDictionary<K,V>` và `SortedSet<T>` gặp đúng nguyên nhân này nhưng ném **`ArgumentException`** ("At least one object must implement IComparable") thay vì `InvalidOperationException`, vì cây đỏ-đen bên trong gọi thẳng `Comparer<T>.Default.Compare` chứ không qua `Array.Sort`. Dù kiểu exception khác nhau, **cách sửa giống hệt nhau** ở cả ba lớp.

**Cách sửa:** cài `IComparable<T>` lên kiểu khoá (xem 7.6), hoặc truyền một `IComparer<T>` vào constructor của `SortedDictionary`/`SortedList`/`SortedSet`.

### 7.5 Tổng kết: bảng so sánh 3 lớp Sorted
...
### 7.6 Thứ tự tuỳ chỉnh: `IComparer<T>` và `IComparable<T>`
...
```csharp title="Span<T>: slice mảng KHÔNG copy"
// test:run
...
`ReadOnlySpan<T>` là phiên bản **chỉ đọc** của `Span<T>` — không cho ghi qua cửa sổ, thường dùng để slice `string` (vì `string` bất biến) mà không tạo bản sao.

```csharp title="ReadOnlySpan<char>: slice chuỗi KHÔNG tạo string mới"
// test:run
...
foreach (var x in new List<int>(c))           // constructor copy tạo bản sao để duyệt
    if (x % 2 == 0) c.Remove(x);
Console.WriteLine(string.Join(",", c));       // 1,3,5

// Cách 4: gom cần xoá rồi xoá sau vòng lặp
var d = new List<int> { 1, 2, 3, 4, 5 };
var canXoa = new List<int>();
foreach (var x in d)
    if (x % 2 == 0) canXoa.Add(x);            // chỉ đọc, chưa sửa d trong lúc duyệt
...
static int[] Nhan2(IEnumerable<int> xs)
{
    var gapDoi = new List<int>();
    foreach (var x in xs) gapDoi.Add(x * 2);
    return [..gapDoi];   // spread bản List vừa tính vào mảng kết quả
}
...
4. **Đếm phần tử trên `IEnumerable<T>` khi không biết cài đặt bên dưới có gì.** Nếu bạn chỉ có `IEnumerable<T>` (không biết đó là `List`, mảng hay thứ khác), bạn phải tự `foreach` để đếm hoặc kiểm tra "có phần tử nào không" — không có `Count`/`Length` trên interface này (mục 1.1). Chương LINQ (sau chương này) sẽ giới thiệu các phương thức tiện lợi hơn cho việc này.

5. **Sửa collection khi `foreach`.** Ném `InvalidOperationException`. Dùng `RemoveAll`, duyệt ngược bằng index, hoặc duyệt bản sao qua constructor copy (xem mục 11).
