...
boxInt.Value = 100;
Console.WriteLine(boxInt.Value);            // 100
...
*(Mục 6.4 sẽ quay lại `Box<T>` này và thêm một phương thức `Map<TOut>` — sau khi bạn đã học `Func<T,TOut>` là gì.)*
...
```csharp title="Lỗi nếu sai — truyền value type vào where T : class"
// test:skip int vi phạm 'where T : class' -> CS0452
Console.WriteLine(OrDefault<int>(5));

static T? OrDefault<T>(T? value) where T : class
    => value is null ? null : value;
```

`int` là value type, không thoả `where T : class`, nên trình biên dịch từ chối ngay tại lời gọi với lỗi `CS0452` ("The type 'int' must be a reference type in order to use it as parameter 'T'...").

### 4.2. `where T : struct` — T phải là value type (non-nullable)

```csharp title="where T : struct"
// test:run
int? boxedInt = Wrap<int>(42);
Console.WriteLine(boxedInt);              // 42
Console.WriteLine(boxedInt.HasValue);     // True

bool boxedBool = default;
Console.WriteLine(Wrap<bool>(boxedBool)); // False

// T : struct cho phép trả T? nghĩa là Nullable<T>
static T? Wrap<T>(T value) where T : struct => value;
```

`T : struct` ngầm bao gồm cả "non-nullable". `Nullable<T>` (chính là `int?`) đòi hỏi `where T : struct` trong định nghĩa của nó — đó là vì sao kiểu trả về `T?` ở trên hợp lệ: trình biên dịch biết chắc `T` không thể tự nullable, nên `T?` luôn là `Nullable<T>` chứ không mơ hồ.

```csharp title="Lỗi nếu sai — truyền reference type vào where T : struct"
// test:skip string vi phạm 'where T : struct' -> CS0453
Console.WriteLine(Wrap<string>("abc"));

static T? Wrap<T>(T value) where T : struct => value;
```

`string` là reference type, không thoả `where T : struct`, nên trình biên dịch từ chối tại lời gọi với lỗi `CS0453` ("The type 'string' must be a non-nullable value type in order to use it as parameter 'T'...").

**Nâng cao — kết hợp `where T : struct` với một interface có `static abstract` member.** C# 11 (.NET 7+) cho phép interface khai báo **member tĩnh** như một phần hợp đồng generic (`static abstract`/`static virtual`) — khái niệm này được giải thích đầy đủ ở phần "DEEP DIVE — Generics dưới tầng IL/runtime" (mục Generic math, cuối chương). `IParsable<T>` là một interface chuẩn của BCL dùng kỹ thuật đó, cho phép gọi thẳng `T.TryParse(...)` (một static method) ngay trong generic method — điều không thể làm được trước C# 11:

```csharp title="where T : struct, IParsable<T> — static abstract interface member"
// test:run
int? maybe = Parse<int>("123");
Console.WriteLine(maybe);                 // 123
Console.WriteLine(Parse<int>("xyz").HasValue);  // False

// IParsable<T> mở khoá T.TryParse(...) — một static abstract member (C# 11+)
static T? Parse<T>(string s) where T : struct, IParsable<T>
    => T.TryParse(s, null, out var v) ? v : null;
```

```csharp title="Lỗi nếu sai — kiểu không triển khai IParsable<T>"
// test:skip Point không triển khai IParsable<Point> -> CS0315
Console.WriteLine(Parse<Point>("1,2"));
...
`Point` là struct hợp lệ cho `where T : struct`, nhưng không triển khai `IParsable<Point>`, nên không thoả phần ràng buộc interface. Trình biên dịch từ chối tại lời gọi với lỗi `CS0315` ("There is no boxing conversion from 'Point' to 'IParsable<Point>'...").
...
```csharp title="Lỗi nếu sai — kiểu không có ctor rỗng public vào where T : new()"
// test:skip NoParameterlessCtor không có ctor rỗng -> CS0310
Console.WriteLine(Create<NoParameterlessCtor>());

static T Create<T>() where T : new() => new T();

class NoParameterlessCtor
{
    public NoParameterlessCtor(int required) { }
}
```

`NoParameterlessCtor` chỉ có constructor nhận tham số, không thoả `where T : new()`, nên trình biên dịch từ chối tại lời gọi với lỗi `CS0310` ("'NoParameterlessCtor' must be a non-abstract type with a public parameterless constructor...").
...
```csharp title="Lỗi nếu sai — truyền kiểu nullable vào where T : notnull"
// test:run #nullable enable khiến 'int?' vi phạm notnull -> CS8714 (WARNING, vẫn biên dịch và chạy)
#nullable enable
Console.WriteLine(KeyCount<int?, int>(new System.Collections.Generic.Dictionary<int?, int>()));
...
`int?` (`Nullable<int>`) là kiểu nullable, vi phạm `where TKey : notnull`. Khác với các ràng buộc khác (vốn chặn cứng tại compile-time bằng lỗi), `notnull` chỉ được trình biên dịch kiểm tra trong ngữ cảnh nullable: nó phát ra **cảnh báo** `CS8714` ("The type 'int?' may not be used as a type argument...") chứ **không** chặn biên dịch — code trên vẫn build và chạy được, in ra `0`. Muốn biến đây thành lỗi cứng, cần bật `<WarningsAsErrors>` hoặc `<Nullable>enable</Nullable>` kèm treat-warnings-as-errors trong `.csproj`.
...
```csharp title="Lỗi nếu sai — int? không phải unmanaged"
// test:skip int? (Nullable<int>) vi phạm 'where T : unmanaged' -> CS8377
using System.Runtime.CompilerServices;
...
`int?` (`Nullable<int>`) tuy bản chất là struct blittable, nhưng trình biên dịch **đặc cách cấm** `Nullable<T>` thoả `unmanaged` (dù cấu trúc bộ nhớ giống hệt một struct blittable tự viết). Lỗi tại lời gọi là `CS8377` ("The type 'int?' must be a non-nullable value type, along with all fields at any level of nesting, in order to use it as parameter 'T'...").
...
```csharp title="Lỗi nếu sai — truyền lớp không kế thừa Shape"
// test:skip Cat không kế thừa Shape -> CS0311
Console.WriteLine(Describe(new Cat()));
...
`Cat` không kế thừa `Shape`, không có phép chuyển đổi tham chiếu ngầm định (implicit reference conversion) sang `Shape`, nên trình biên dịch từ chối tại lời gọi với lỗi `CS0311` ("There is no implicit reference conversion from 'Cat' to 'Shape'...").
...
```csharp title="Lỗi nếu sai — struct không triển khai IComparable<T>"
// test:skip Coord không triển khai IComparable<Coord> -> CS0311
Console.WriteLine(Max(new[] { new Coord(1, 1), new Coord(2, 2) }));
...
`Coord` không triển khai `IComparable<Coord>`, nên không có phép chuyển đổi hợp lệ để thoả ràng buộc `where T : IComparable<T>`. Trình biên dịch từ chối tại lời gọi với lỗi `CS0311` ("There is no implicit reference conversion from 'Coord' to 'System.IComparable<Coord>'...").
...
```csharp title="Lỗi nếu sai — sai thứ tự ràng buộc"
// test:skip new() đứng trước interface, sai thứ tự bắt buộc -> CS0401
static T Build<T>() where T : new(), IStore => new T();

interface IStore { void Save(string s); int Count { get; } }
```

`new()` phải luôn đứng **cuối cùng**; đặt trước `IStore` khiến trình biên dịch từ chối với lỗi `CS0401` ("The new() constraint must be the last constraint specified"). Tương tự, nếu bạn đặt `class`/`struct` không phải đầu tiên (ví dụ `where T : IStore, class`), lỗi sẽ là `CS0449` ("The 'class' or 'struct' constraint must come before any other constraints").
...
```csharp title="Lỗi nếu sai — U không phải T hoặc con của T"
// test:skip Cat không phải Animal hay con của Animal -> CS0311
var animals = new System.Collections.Generic.List<Animal>();
AddInto<Animal, Cat>(animals, new Cat());
...
`Cat` không kế thừa `Animal`, nên không thoả ràng buộc `where U : T` khi `T = Animal`. Trình biên dịch từ chối tại lời gọi với lỗi `CS0311` ("There is no implicit reference conversion from 'Cat' to 'Animal'...").
...
!!! note "Bảng này chỉ để TRA CỨU nhanh, chưa phải bài học"
    Bốn dòng cuối (`class?`, `Enum`, `Delegate`, `allows ref struct`) mới chỉ liệt kê Ý NGHĨA — đây là "mục lục xem trước", KHÔNG phải chỗ học khái niệm lần đầu. Ngay bên dưới, mục 4.11 dạy đầy đủ cả 4 ràng buộc này với định nghĩa, ví dụ chạy được, và (nếu có) lỗi khi dùng sai. Nếu bạn chưa từng gặp 4 ràng buộc này, hãy đọc 4.11 trước khi coi bảng trên là "đã hiểu".
...
```csharp title="Lỗi nếu sai — int không phải enum"
// test:skip int vi phạm 'where T : Enum' -> CS0315
Console.WriteLine(NamesOf<int>());

static string NamesOf<T>() where T : Enum
    => string.Join(", ", Enum.GetNames(typeof(T)));
```

`int` là value type nhưng không phải `enum`, nên không có phép chuyển đổi boxing hợp lệ sang `System.Enum`. Trình biên dịch từ chối tại lời gọi với lỗi `CS0315` ("There is no boxing conversion from 'int' to 'System.Enum'...").
...
```csharp title="Lỗi nếu sai — string không phải delegate"
// test:skip string vi phạm 'where T : Delegate' -> CS0311
Console.WriteLine(CountHandlers("không phải delegate"));

static int CountHandlers<T>(T d) where T : Delegate
    => d.GetInvocationList().Length;
```

`string` là reference type nhưng không kế thừa `System.Delegate`, nên không có phép chuyển đổi tham chiếu ngầm định thoả ràng buộc. Trình biên dịch từ chối tại lời gọi với lỗi `CS0311` ("There is no implicit reference conversion from 'string' to 'System.Delegate'...").

**`where T : allows ref struct`** (C# 13) — cho phép `T` được đóng bằng một *ref struct* như `Span<T>`/`ReadOnlySpan<T>`, thứ mà trước C# 13 không bao giờ được dùng làm tham số kiểu (vì ref struct chỉ được sống trên stack, generic thông thường không đảm bảo điều đó). Ràng buộc này nói với trình biên dịch "tôi biết `T` có thể là ref struct, đừng cấm".

```csharp title="where T : allows ref struct"
// test:run
Span<int> nums = stackalloc int[] { 3, 1, 2 };
Span<int> same = Identity(nums);       // T được PHÉP là Span<int> nhờ 'allows ref struct'
Console.WriteLine(same.Length);        // 3
Console.WriteLine(same[0]);            // 3

// KHÔNG có 'allows ref struct' thì T KHÔNG thể là Span<int> — lỗi ngay tại lời gọi (CS9244)
static T Identity<T>(T value) where T : allows ref struct => value;
```

`allows ref struct` không tự nó mở khoá member gì trên `T` — nó chỉ **gỡ bỏ lệnh cấm** dùng ref struct (như `Span<int>`) làm đối số kiểu, cho phép `T` "đi xuyên qua" một API generic (nhận vào, trả ra) mà không cần boxing (vốn dĩ **không thể** với ref struct). Muốn gọi member cụ thể của `Span<T>` bên trong thân method tổng quát, bạn cần thiết kế khác (nhận thẳng `Span<T>` thay vì `T` chung chung). Trước C# 13, dòng `Identity(nums)` với `nums` là `Span<int>` sẽ bị chặn ngay tại chỗ gọi vì ref struct bị cấm làm đối số kiểu (`CS9244`).
...
// test:run
#nullable enable
Console.WriteLine(Describe<string>(null));     // (không có giá trị)
Console.WriteLine(Describe("xin chào"));        // xin chào

// class? chấp nhận CẢ string? lẫn string mà KHÔNG bị warning nullability
static string Describe<T>(T value) where T : class?
    => value is null ? "(không có giá trị)" : value.ToString()!;
```

So với `where T : class` (mục 4.1): `class` buộc trình biên dịch coi `T` là non-nullable (truyền `null` trực tiếp — không qua `T?` — sẽ phát sinh warning nullability trong ngữ cảnh `#nullable enable`), còn `class?` nói rõ "tôi chấp nhận `T` có thể là dạng nullable ngay từ đầu", nên gọi `Describe<string?>(null)` hay `Describe<string>(null)` đều không bị cảnh báo.
...
```csharp title="Lỗi nếu sai — so sánh == với T không ràng buộc"
// test:skip T không ràng buộc, không đảm bảo có toán tử == -> CS0019
static bool IsDefault<T>(T value) => value == default(T);
```

Với `T` hoàn toàn không ràng buộc (không `class`, không `struct`, không interface có `==`), trình biên dịch không biết `T` có toán tử `==` hay không, nên từ chối biên dịch với lỗi `CS0019` ("Operator '==' cannot be applied to operands of type 'T' and 'T'"). Cách đúng — dùng `EqualityComparer<T>.Default.Equals(value, default!)` như đã nêu ở trên.
...
// test:run
var ages = new System.Collections.Generic.List<Person> { new("An", 30), new("Bình", 25) };
ages.Sort();                                     // dùng IComparable (theo tuổi)
Console.WriteLine(string.Join(", ", ages.Select(p => p.Name)));   // Bình, An
...
**`IComparer<T>` một mình** — một lớp so sánh riêng, KHÔNG cần sửa `Person`, truyền vào `List<T>.Sort(IComparer<T>)`:

```csharp title="IComparer<T>"
// test:run
var names = new System.Collections.Generic.List<Person> { new("Cường", 40), new("An", 30) };
names.Sort(new ByNameComparer());                // dùng IComparer (theo tên)
Console.WriteLine(string.Join(", ", names.Select(p => p.Name)));  // An, Cường
...
Khi kiểu phần tử **không** implement `IComparable<T>` và bạn gọi `Sort()` không tham số, lỗi chỉ lộ ra lúc chạy:

```csharp title="Sort() không IComparable -> InvalidOperationException"
// test:run
...
try
{
    boxes.Sort();   // Box KHÔNG implement IComparable<Box> -> biên dịch OK, NỔ lúc chạy
}
catch (InvalidOperationException ex)
{
    Console.WriteLine($"Nổ: {ex.GetType().Name}");
}
...
```text title="Kết quả"
Nổ: InvalidOperationException
```

`List<T>.Sort()` không tham số dùng `Comparer<T>.Default`, mà `Comparer<T>.Default` lại tìm `IComparable<T>`/`IComparable` trên `T` lúc chạy — không có thì ném `InvalidOperationException` kèm thông báo "không triển khai IComparable". Muốn tránh lỗi này mà không sửa lớp gốc, dùng `Sort(IComparer<T>)` như ví dụ `IComparer<T>` ở trên.

Kết hợp cả hai trong cùng một kiểu: `Person` vừa tự có thứ tự mặc định (`IComparable<T>`, theo tuổi) vừa cho phép sắp xếp kiểu khác qua `IComparer<T>` (theo tên) mà không cần sửa lớp:

```csharp title="IComparable<T> + IComparer<T> kết hợp"
// test:run
...
Bạn đã thấy `Action<Animal>` và các delegate tự khai báo (`Transformer<T, TOut>` sắp tới) — nhưng chưa gọi tên khái niệm: `Func<T, TOut>` và `Action<T>` **chính là generic delegate** — một kiểu delegate được khai báo với tham số kiểu, giống hệt generic class/interface nhưng cho "kiểu hàm". `Func<T, TOut>` tương đương một `delegate TOut FuncLike<T, TOut>(T arg);` do BCL định nghĩa sẵn cho bạn (`Func<>` có nhiều bản từ 0 đến 16 tham số, `Action<>` tương tự nhưng không trả về giá trị).
...
### 6.5. `Box<T>.Map` — áp dụng `Func<>` vừa học vào `Box<T>` (mục 2.1)

Giờ đã biết `Func<T, TOut>` là gì, ta mở rộng `Box<T>` ở mục 2.1 với một phương thức generic `Map<TOut>`: nhận một `Func<T, TOut>` để "biến đổi" giá trị bên trong và trả về một `Box<TOut>` mới.

```csharp title="Box<T>.Map — áp dụng Func vừa học"
// test:run
var boxInt = new Box<int>(42);
...
var boxStr = new Box<string>("xin chào");
var lenBox = boxStr.Map(s => s.Length);     // Box<string> -> Box<int>
Console.WriteLine(lenBox.Value);            // 8
...
`Map<TOut>` là một **generic method** lồng trong một **generic class**: `T` đến từ `Box<T>`, còn `TOut` là tham số kiểu riêng của chính `Map`. Vì `Func<T, TOut>` giờ đã quen thuộc (mục 6.4), chữ ký `Map<TOut>(Func<T, TOut> f)` không còn là "phép thuật" — nó chỉ là một generic delegate nhận `T`, trả `TOut`.
...
```csharp title="Lỗi nếu sai — tạo instance trực tiếp từ open generic type"
// test:run
Type open = typeof(System.Collections.Generic.List<>);
try
{
    Activator.CreateInstance(open);   // open type -> KHÔNG tạo được instance
}
...
```text title="Kết quả"
Nổ: ArgumentException
```

Không thể `new List<>()` hay `Activator.CreateInstance` trực tiếp trên một **open generic type** — CLR chưa biết cấp phát bao nhiêu bộ nhớ, bố trí trường ra sao vì tham số kiểu chưa được điền. Phải đóng type trước bằng `MakeGenericType(typeof(int))` (như ví dụ trên) rồi mới `Activator.CreateInstance` trên type **đã đóng**. Đây là lỗi runtime (`ArgumentException`), không phải lỗi biên dịch, vì `Activator.CreateInstance(Type)` nhận `Type` là giá trị runtime — trình biên dịch không kiểm tra được trước.
