...
CREATE TABLE don_hang (
    id             integer GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    khach_hang_id  integer REFERENCES khach_hang(id),   -- KHÔNG NOT NULL: cần cho phép NULL để minh hoạ cạm bẫy ở mục 11.3
...
HAVING AVG(d.so_tien) > (             -- <-- điền subquery vô hướng ở đây
    /* trung bình chung mọi đơn có giá trị VÀ có gắn khách (loại đơn mồ côi) */
);
```
...
??? success "Lời giải"
    ...
    GROUP BY k.id, k.ten
    HAVING AVG(d.so_tien) > (SELECT AVG(so_tien) FROM don_hang WHERE so_tien IS NOT NULL AND khach_hang_id IS NOT NULL);
    ```
    Vì sao: điều kiện so sánh dựa trên `AVG(d.so_tien)` — một hàm tổng hợp — nên bắt buộc đặt trong `HAVING`, không thể đặt trong `WHERE` (xem mục 9.2). `JOIN` (không `LEFT`) là đủ ở đây vì đề chỉ quan tâm khách **đã có** đơn hợp lệ. Subquery tính trung bình chung phải lọc thêm `khach_hang_id IS NOT NULL` — nếu không, đơn hàng "mồ côi" (không gắn khách nào, minh hoạ ở mục 11.3) vẫn lọt vào trung bình chung dù nó không thuộc về khách nào cả, kéo lệch kết quả so sánh.
