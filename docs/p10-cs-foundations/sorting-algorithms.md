...
**Merge Sort là stable** (xem định nghĩa "stable" ở mục 6) vì bước gộp dùng so sánh `<=`: khi hai phần tử bằng nhau, phần tử từ nửa trái (đến trước trong mảng gốc) luôn được chọn trước.
...
**Định nghĩa "stable" (ổn định):** một thuật toán sắp xếp là **stable** nếu nó **giữ nguyên thứ tự tương đối** của các phần tử có giá trị **bằng nhau** so với thứ tự ban đầu trong mảng gốc. Ví dụ: sắp xếp danh sách nhân viên theo `Tuoi`, nếu 2 nhân viên cùng tuổi 30 mà người A đứng trước người B trong mảng gốc, thuật toán stable đảm bảo A vẫn đứng trước B sau khi sắp; thuật toán không-stable thì không đảm bảo điều này.
...
**Tiếp theo →** phân tích độ phức tạp thuật toán nâng cao
