...
Chọn một chương (hoặc "🔀 Trộn toàn bộ chương trình" để luyện interleaving — câu hỏi rút ngẫu nhiên từ nhiều chương khác nhau, giúp phân biệt khái niệm tốt hơn so với ôn từng chương một cách tuần tự), hệ thống lấy ngẫu nhiên vài câu hỏi từ đúng bộ Q&A tương ứng (cùng nguồn `data/questions.yml` đang bảo vệ bởi gate `qa_lint.py`) — không có gì được viết thêm ở đây, chỉ đọc lại dữ liệu đã có. Đoán trước, rồi bấm để xem đáp án.
...
(function () {
  ...
  var MIX_ALL = "__mix_all__";

  function populateSelect(select, nodes, titles) {
    select.innerHTML = "";
    var mixOpt = document.createElement("option");
    mixOpt.value = MIX_ALL;
    mixOpt.textContent = "🔀 Trộn toàn bộ chương trình";
    select.appendChild(mixOpt);
    nodes
      .slice()
      .sort(function (a, b) {
        return (titles[a] || a).localeCompare(titles[b] || b, "vi");
      })
      .forEach(function (node) {
        var opt = document.createElement("option");
        opt.value = node;
        opt.textContent = titles[node] || node.replace(/-/g, " ");
  ...
  function start() {
    ...
    var pool;
    if (node === MIX_ALL) {
      pool = Object.keys(data.questions).reduce(function (all, key) {
        return all.concat(data.questions[key]);
      }, []);
    } else {
      pool = (data && data.questions[node]) ? data.questions[node] : [];
    }
  ...
  loadData().then(function (json) {
    data = json;
    var nodeSelect = document.getElementById("quiz-node");
    populateSelect(nodeSelect, Object.keys(data.questions), data.titles);
